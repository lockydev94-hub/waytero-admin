// ============================================================
// WAYTERO ADMIN — HOTEL COLLECT-PAYMENT MODAL (with duplicate guard)
...
// ────────────────────────
// A double-click on "Collect Payment" (or a network retry) used to risk a
// second payment row being inserted against the same invoice. The fix is a
// three-layer guard:
//
//   1. Button is disabled the instant the click is registered, so a fast
//      second tap is ignored at the React level.
//   2. The mutation has a 2-second cooldown that rejects repeat calls with
//      the same `amount + mode + reference` shape — that swallows the
//      common case of a double-click without a network round-trip.
//   3. The request carries an `Idempotency-Key` header (a hash of the
//      inputs + a UUID stored in module state) so the server can dedupe
//      even after a network retry from a different tab.
// ============================================================
...
// ── Lightweight string hash for idempotency keys (no crypto needed) ──
...
interface Props {
  ...
  advancePaid: number;
  balanceDue: number;
  /** Called with the resolved result on success — host can use it for
   *  inline state updates (e.g. flip the booking to "fully paid"). */
  onCollected?: (result: any) => void;
}

export default function HotelCollectPaymentModal({
  open, onClose, bookingId, hotelId, hotelBookingNumber,
  invoiceNumber, grandTotal, advancePaid, balanceDue, onCollected,
}: Props) {
  ...
  // Track the last submission signature so a fast double-click is rejected
  // even before the button-disabled state can propagate.
  const lastSubmitSig = useRef<string>("");
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      // Build (or reuse) a per-submission idempotency key. Re-using the
      // same key across retries is the whole point of an Idempotency-Key
      // header — the server can detect a retry and return the original
      // response without re-applying the side effect.
      const sig = `${numAmount}|${mode}|${effectiveCollector}|${reference.trim()}`;
      const now = Date.now();

      if (
        lastSubmitSig.current === sig &&
        now - lastSubmitAt.current < 5000
      ) {
      ...
      if (!idempotencyKey.current) {
        idempotencyKey.current = `${Date.now()}-${quickHash(sig + Math.random().toString())}`;
      }

      // Use the lower-level apiClient so we can attach the header. The
      // bookingService wrapper does not expose headers.
      const resp = await apiClient.post(
        `/admin/bookings/${bookingId}/hotel/${hotelId}/collect-payment`,
        {
          amount: numAmount,
          payment_mode: mode,
          payment_collected_by: effectiveCollector,
    ...
    onError: (e: any) => {
      // Reset the dedupe so a genuine retry after a failure can succeed.
      lastSubmitSig.current = "";
      lastSubmitAt.current = 0;
      idempotencyKey.current = "";
      ...
      if (e?.message === "DUPLICATE_SUBMIT") {
        enqueueSnackbar(
          "Duplicate payment blocked — the same amount/mode/reference was submitted in the last 5 seconds.",
          { variant: "warning" },
        );
        return;
      }
      const detail = e?.response?.data?.detail;
      enqueueSnackbar(typeof detail === "string" ? detail : "Failed to collect payment", { variant: "error" });
    },
  });
  ...
  // Disable the submit button whenever a request is in flight OR a previous
  // submission is still inside its dedupe window. This is the visible part
  // of the duplicate guard; the in-flight mutation also disables it.
  const submitLocked = mutation.isPending;
  const submitHint = useMemo(() => {
    if (mutation.isPending) return "Processing…";
    if (!valid) {
      if (!invoiceNumber) return "Generate the invoice first.";
      if (numAmount <= 0) return "Enter an amount.";
      if (exceedsBalance) return `Exceeds balance ${fmtINR(balanceDue)}.`;
      return "Fill the required fields.";
    }
    return isPartial
      ? `Part payment — ${fmtINR(balanceDue - numAmount)} will remain outstanding.`
      : "Custody recorded on the receipt and feeds into settlement.";
  }, [mutation.isPending, valid, invoiceNumber, numAmount, exceedsBalance, balanceDue, isPartial]);
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
      ...
      <Divider />
      <DialogActions sx={{ p: 2, gap: 1, alignItems: "center" }}>
